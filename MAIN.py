...
def predict(company_name, days_to_predict):
    style.use('ggplot')
    try:
        df= pd.read_csv(company_name+".csv", header=0, index_col='Date', parse_dates=True)
    ...
    ##    plt.show()
        ...
        messagebox.showerror(title='Data not found',message='The data for the company listed was not found! Please download the data from finance.google.com!')
